...
/**
 * Dialog used for choosing a GWT Module. When the OK button is pressed, an IModule is returned to the caller.
 */
...
public class ModuleSelectionDialog extends FilteredItemsSelectionDialog {
	 ...
	 * 
	 * This code was adapted from the FilteredTypesSelectionDialog.TypeItemDetailsLabelProvider class.
	 */
	private class ModuleItemDetailsLabelProvider extends ModuleItemLabelProvider {
		...
		public Image getImage(Object element) {
			...
			if (!module.isBinary()) {
				...
				try {
					/*
					 * TODO: Move logic for retrieving a module's package fragment into IModule.
					 */
					...
					GWTPluginLog.log(IStatus.WARNING, IStatus.OK,
							"Unable to retrieve image for module " + module.getQualifiedName() + " package.", e);
				}
			}
	 ...
	 * 
	 * This code was adapted from the FilteredTypesSelectionDialog.TypeItemLabelProvider class.
	 */
	private static class ModuleItemLabelProvider extends LabelProvider {

		/*
		 * Provides basic labels for workbench resources that implement IWorkbenchAdapter
		 */
	...
	/**
	 * Filters the items in the dialog list based on the pattern entered in the filter text field of the dialog.
	 */
	private class ModuleItemsFilter extends ItemsFilter {
		...
		public ModuleItemsFilter() {
			...
			/*
			 * If there is no filter pattern present, initialize the pattern to '*'. This has the nice property of
			 * pre-populating the dialog list with all possible matches when it is first shown.
			 */
			if (patternMatcher.getPattern() == null || patternMatcher.getPattern().length() == 0) {
		...
		public boolean isConsistentItem(Object item) {
			/*
			 * TODO: For now, let's say that everything is consistent. That is, don't worry about file deletions
			 * happening while the dialog is open (it is a modal dialog; can this happen? What if there are changes from
			 * the shell? how does this thing know about them?).
			 */
			return true;
		}
	 ...
	 * 
	 * This code was adapted from the FilteredTypesSelectionDialog.TypeSearchPattern class.
	 */
	...
	/*
	 * Key used for the storage/retrieval of this dialog's settings from the IDE's persistent data store.
	 */
	private static final String DIALOG_SETTINGS = GWTPlugin.PLUGIN_ID + ".dialogs.ModuleInheritsSelectionDialog";
	 ...
	 * @param showModulesInJars true if modules that are located in jars on the project's classpath should be displayed
	 * 
	 * @return the IModule corresponding to the module chosen from the list by pressing the OK button, null if CANCEL
	 *         was pressed.
	 */
	 ...
	 * 
	 * If the module file is located in a JAR, then the absolute path of the JAR on the file system is returned.
	 */
	...
	protected void fillContentProvider(AbstractContentProvider contentProvider, ItemsFilter itemsFilter,
		...
		/*
		 * TODO: Should probably try and use the progressMonitor appropriately here. If we end up searching the whole
		 * workspace for modules, this could take a while, and we need some accurate indication of progress.
		 */

		IModule[] projectModules = ModuleUtils.findAllModules(javaProject, showModulesInJars);
	...
	protected IStatus validateItem(Object item) {
		/*
		 * There is nothing to validate. As long as an item has been selected when the OK button is pressed (and the
		 * dialog will not let you press the OK button unless an item has been selected), there is nothing else to
		 * check.
		 */
		return new Status(Status.OK_STATUS.getSeverity(), GWTPlugin.PLUGIN_ID, 0, "", null);
	}
